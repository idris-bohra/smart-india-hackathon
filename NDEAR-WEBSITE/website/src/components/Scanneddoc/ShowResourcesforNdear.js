...
export default function ShowResourceofschool() {
  ...
  useEffect(() => {
    ...
    async function fun(){
        ...
        axios.get(`http://localhost:${process.env.REACT_APP_SERVER}/getResourcesForNdear/${params.id}`,  {headers : {"Content-Type" : "application/json" }}).then((result)=>{
  ...
  return (
    <div>
      {loading==false ?  attendence==false ? <div style={{display:'flex' ,width: "100%",height: "50vh", flexDirection:'row', alignItems:"center", justifyContent:'space-around'}}>
        <h1>NO RESOURCE RECORDS UPDATED</h1>
    ...
    <div className='attendance_links'>
      {mystate.resource.map((el,key)=>{
          return(<>
          <div>
            <img key={key} style={{width:'150px', height:'150px'}} src={txt} alt=""/>
            <h6><a className={'linkname'} target="_blank" href={el}>Record {key+1}</a></h6>
          </div>
          </>)
      })}
    </div>
